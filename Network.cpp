...
bool NetworkServer::Start(int port, std::function<void(const unsigned char*, int)> dataCallback) {
...
void NetworkServer::ClientHandler(SOCKET clientSocket) {
    ...
    while (running) {
        int bytesReceived = recv(clientSocket, (char*)buffer.data(), (int)buffer.size(), 0);
        if (bytesReceived > 0) {
            if (onDataReceived) {
